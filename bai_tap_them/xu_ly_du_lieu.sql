use furama;
-- Câu 2 : Hiển thị thông tin của tất cả nhân viên có trong hệ thống có tên bắt đầu là một trong các ký tự “H”, “T” hoặc “K” và có tối đa 15 ký tự.
...
select kh.ma_khach_hang, kh.ho_ten, COUNT(hd.ma_khach_hang) as so_lan_dat_phong from khach_hang kh join loai_khach lk using (ma_loai_khach)
...
if (hdct.ma_hop_dong is null, dv.chi_phi_thue, (dv.chi_phi_thue + sum(hdct.so_luong * dvdk.gia ))) AS tong_tien	from khach_hang kh join loai_khach lk using(ma_loai_khach)
...
join loai_dich_vu ldv  using(ma_loai_dich_vu)
right join hop_dong hd using(ma_dich_vu)
where dv.ma_dich_vu in (select dv.ma_dich_vu from dich_vu dv
join hop_dong hd using(ma_dich_vu)
where year(hd.ngay_lam_hop_dong) = '2020')
and dv.ma_dich_vu not in (select dv.ma_dich_vu from dich_vu dv
join hop_dong hd using(ma_dich_vu)
...
select month(hd.ngay_lam_hop_dong) as `month` ,count(ma_khach_hang) as `so_luong_khach_hang` from hop_dong hd
where  (year(hd.ngay_lam_hop_dong)='2021')
group by `month`
order by `month`;
...
select hd.ma_hop_dong ,nv.ho_ten, kh.ho_ten, kh.so_dien_thoai, dv.ten_dich_vu, IFNULL( SUM(hdct.so_luong), 0) AS so_luong_dich_vu_di_kem, hd.tien_dat_coc from hop_dong hd 
...
-- Câu 15:	Hiển thi thông tin của tất cả nhân viên bao gồm ma_nhan_vien, ho_ten, ten_trinh_do, ten_bo_phan, so_dien_thoai, dia_chi mới
...
where 
ma_nhan_vien not in (select * from (select hd.ma_nhan_vien from nhan_vien nv
...
(select kh.ma_khach_hang,if (hdct.ma_hop_dong is null, dv.chi_phi_thue, (dv.chi_phi_thue + sum(hdct.so_luong * dvdk.gia ))) AS tong_tien from khach_hang kh
join loai_khach lk using(ma_loai_khach) 
left join hop_dong hd using(ma_khach_hang)
left join dich_vu dv using(ma_dich_vu)
left join hop_dong_chi_tiet hdct using(ma_hop_dong)
...
where ma_khach_hang in (select kh.ma_khach_hang from khach_hang kh left join hop_dong hd using(ma_khach_hang)
where year(hd.ngay_lam_hop_dong) < 2021 );

-- Câu 19:	Cập nhật giá cho các dịch vụ đi kèm được sử dụng trên 10 lần trong năm 2020 lên gấp đôi.
update dich_vu_di_kem dvdk
set gia = gia * 2 
where ten_dich_vu_di_kem = (select result.ten_dich_vu_di_kem from (select ma_dich_vu_di_kem,ten_dich_vu_di_kem from dich_vu_di_kem  
...
union all select kh.ma_khach_hang as id, kh.ho_ten, kh.email, kh.so_dien_thoai, kh.ngay_sinh, kh.dia_chi from khach_hang kh;